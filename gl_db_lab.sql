...
SELECT c.CAT_ID, c.CAT_NAME, p.PRO_NAME, sp.SUPP_PRICE AS Price
FROM category c
JOIN product p ON c.CAT_ID = p.CAT_ID
JOIN supplier_pricing sp ON p.PRO_ID = sp.PRO_ID
WHERE (sp.PRO_ID, sp.SUPP_PRICE) IN (
    SELECT sp_inner.PRO_ID, MIN(sp_inner.SUPP_PRICE)
    FROM supplier_pricing sp_inner
    GROUP BY sp_inner.PRO_ID
)
ORDER BY c.CAT_ID;
